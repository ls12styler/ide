...
# Create a user and grouup that matches the host
HOST_USER_ID=`stat -c %u /workspace`
HOST_GROUP_ID=`stat -c %g /workspace`
...
if [ -S "/var/run/docker.sock" ]; then
    ...
    groupadd -g "$HOST_DOCKER_SOCKET_GROUP_ID" docker
